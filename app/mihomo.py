        ...
        try:
            ...
            with open(fileName, 'a') as f:
                ...
                if isLite:
                    ...
                    f.write("  # Description: 适用于 Clash Meta(Mihomo) 的去广告合并规则，每 12 个小时更新一次。规则源：%s。Lite 版仅针对国内域名拦截。\n"%(self.sourceRule))
                else:
                    f.write("  # Title: AdBlock Clash Meta(Mihomo)\n")
                    f.write("  # Description: 适用于 Clash Meta(Mihomo) 的去广告合并规则，每 12 个小时更新一次。规则源：%s。\n"%(self.sourceRule))
                f.write("  # Homepage: %s\n"%(self.homepage))
