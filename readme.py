...
class ReadMe(object):
    ...
    def __subscribeLink(self, fileName:str, url:str=None):
        ...
        if url:
            ...
            link += " [原始链接](https://raw.githubusercontent.com/QSDR2s1d/AdRules/main/rules/%s) |"%(fileName)
        
        for i in range(1, len(self.proxyList)):
            proxy = self.proxyList[i]
            if proxy.startswith("https://gcore.jsdelivr.net/"):
                link += " [加速链接%d](%s/QSDR2s1d/AdRules@main/rules/%s) |"%(i, proxy, fileName)
            else:
                link += " [加速链接%d](%s/https://raw.githubusercontent.com/QSDR2s1d/AdRules/main/rules/%s) |"%(i, proxy, fileName)
